...
const createTransfers = async (
  ...
  const fursatFee = parseInt(
    process.env.NEXT_PUBLIC_FURSAT_SERVICE_FEE_PER || "5"
  );
  const fursatFeePer = fursatFee / 100;
...
const createCoHostTransfers = async (
  ...
  const cohostBasePayment =
    ...
    hostBasePayment -
    priceBreakUp.basePrice * fursatFeePer;

  const cohostextraGuestCharges =
    priceBreakUp.extraGuestCharges -
    hostExtraGuestCharges -
    priceBreakUp.extraGuestCharges * fursatFeePer;
